...
def main():
    ...
    arg.add_argument(
        "-t", "--timeout", default=1.0, type=float, help="time limit for one test"
    )
    arg.add_argument(
        "-m",
        "--more",
        action="store_true",
        help="agree more report than ans, only for advance test",
    )
    arg.add_argument(
        "--color",
    ...
    for dir in os.listdir("extend"):
        if (
            (group == 0 and not dir.startswith("not"))
            or dir == str(group)
            or (group != 0 and dir.startswith("not") and dir != f"not{group}")
        ):
...
def test_one(file: str):
    ...
    if ans:
        ...
        if err in ans:
            ...
            nearest := max([i for i in ans if set(i).issubset(err)] + [[]], key=len)
        ) != []:
            print(
                f"[{file}] {color('yellow', 'FalsePositive')}: got {err}, should NOT report {[i for i in err if i not in nearest]}"
            )
...
def test_advance_one(file: str):
    ...
    if anserr := analyse_out(ans, False):
        ...
        if ac:
            ...
            print(
                f"[{file}] {color('yellow', 'FalsePositive')}: got {outerr}, should NOT report {[i for i in outerr if i not in anserr]}"
            )
            return "FalsePositive"
        else:
